# %% [markdown]
# # MeloMatch — Kaggle T4×2 (Fine-Tuned: Qwen3-8B via vLLM)
# **CSC5051 NLP Final Project — "Don't Recommend Me Cats"**
#
# This notebook runs the **fine_tuned backbone** conditions on Kaggle T4×2 (2×16GB).
# Uses vLLM with tensor_parallel_size=2 for multi-GPU inference.
#
# Runs: 4 signals × 3 methods × 1 backbone (fine_tuned) = 12 conditions + baseline
#
# For zero_shot backbone + ablations, use `melomatch_colab.py` on Colab A100.
...
BACKBONES = ["fine_tuned"]  # zero_shot runs on Colab A100
...
# Paths — adjust for your Kaggle dataset upload
DATA_DIR = Path("/kaggle/input/melopreference")
...
# Load fine-tuned model via vLLM (fp16 on T4×2, ~16GB VRAM across 2 GPUs)
ft_llm = VllmLLM(
    FINE_TUNED_MODEL,
...
backbone_llms = {"fine_tuned": ft_llm}
...
nli_filter = NLIFilter(ft_llm, threshold=NLI_THRESHOLD)
profiler = PreferenceProfiler(llm=ft_llm, use_llm=True)

print("✓ Indices built, retriever + modules ready.")

# %% [markdown]
# ## 8. Run Main Experiment (4 signals × 3 methods × fine_tuned)
...
save_metrics(all_metrics, str(OUTPUT_DIR / "metrics_fine_tuned.json"))
print(f"\n✓ Fine-tuned experiment complete. {len(all_metrics)} conditions evaluated.")

# %% [markdown]
# ## 9. Visualization & Summary
# Note: Ablations run on Colab only (zero_shot backbone). Merge results offline.
...
def plot_results(all_metrics):
    ...
    plt.tight_layout()
    plt.savefig(str(OUTPUT_DIR / "signal_comparison_fine_tuned.png"), dpi=150)
    plt.show()

plot_results(all_metrics)

print("\n" + "=" * 80)
print("RESULTS SUMMARY (fine_tuned backbone)")
print("=" * 80)
